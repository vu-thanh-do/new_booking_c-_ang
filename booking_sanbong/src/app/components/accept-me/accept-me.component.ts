...
@Component({
  selector: 'app-accept-me',
  templateUrl: './accept-me.component.html',
  styleUrls: ['./accept-me.component.scss']
})
export class AcceptMeComponent implements OnInit {
  user: any;
  dataInvitMe: any[] = [];
  teamAll: any[] = [];
  ...
  getInvitV2() {
    this.TeamserviceService.getInvitV2(this.idMyteam).subscribe((data: any) => {
      var newData = data.filter((items: any) => {
        ...
        return items.accepted == true && this.idMyteam == items.inviteTeamId;
      });
